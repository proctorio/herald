...
| D7 | Popup auto-play on open | REVERSED 2026-09-24 (owner decision): reading starts on the toolbar click again, as upstream. The click is itself the explicit user action and the only activeTab grant, so exam integrity is unchanged. Removing auto-play left an idle popup and an empty popout window that QA reported as a broken extension (P0, 2026-09-20). Original position: remove, reading starts only on explicit user action. |
