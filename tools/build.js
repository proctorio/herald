...
// Channel icons. The committed icons (img/icon-*.png, the app mark, and
// img/action-*.png, the horn-only toolbar mark) ARE the dev-stamped sets,
// pre-compiled by the internal icon tooling, so a raw checkout or a default
// local build side-loads with the dev logo and no build step is needed. img/prod holds the clean store art; the
...
const channel = process.env.HERALD_CHANNEL ?? (process.env.TF_BUILD ? "prod" : "dev");

if (!["dev", "prod"].includes(channel))
{
	throw new Error(`Unknown HERALD_CHANNEL "${channel}"; valid channels: dev, prod`);
}
...
if (channel === "prod")
{
	for (const name of ["icon", "action"])
	{
		...
		{
			copyFileSync(`dist/img/prod/${name}-${size}.png`, `dist/img/${name}-${size}.png`);
		}
	}
}
